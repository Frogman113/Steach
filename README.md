...
이러한 문제를 해결하고자 처음에는 기존 HTTP 방식에서 전송 주기를 짧게 설정하여 녹음을 종료하기 전에 음성 데이터의 일부를 전송하는 방법으로 시도하려 했습니다.  
하지만 여전히 녹음 기능이 종료되어야 서버에 음성 데이터를 전송할 수 있었고, 고객과 나눈 대화의 부분을 잘 나누어서 보내야 AI가 맥락을 파악하여 좋은 답변을 생성하는데 원하는 부분을 끊어서 전송하기가 되지 않았습니다.

종합적으로 판단했을 때 HTTP 방식이 아닌 WebSocket을 통해 실시간으로 음성 데이터를 송수신 하는 것이 현재 프로젝트에 맞다고 생각했습니다.

- 녹음이 진행되는 동안에도 지속적으로 음성 데이터를 전송하여 AI가 실시간으로 처리 가능 => 고객과 대화를 끊지 않아도 됨
- WebSocket은 여러 요청이 아닌 한 번의 연결로 데이터를 계속해서 주고 받을 수 있음 => HTTP 대비 네트워크 성능 개선
- 여러 사용자(영업 사원)가 사용하더라도 WebSocket으로 연결이 되어있기 때문에 HTTP 대비 서버의 부하가 낮아짐

적용 후 문제점  
실시간으로 음성 데이터를 전송하기 위해 데이터를 짧게 쪼개져 전송되다 보니 STT 변환이 부정확해지는 현상이 나타났습니다.  
한 문장이 완전히 입력되기 전에 단어 단위로 인식이 이루어져 의도치 않게 틀린 변환이 이루어지는 경우가 많았고, 문장이 끝나기 전에 AI가 답변을 생성하는 경우도 있다 보니 대화 호름에 맞지 않은 답변을 주어 대화의 맥락이 어색해지는 경우가 발생했습니다.

WebSocket 연결이 끊기면, 실시간으로 전송하던 음성 데이터가 손실될 위험이 있습니다. 그렇게 되면 서버에 완전한 문장 또는 단어가 도착하지 않은 상태에서 STT 변환이 이루어 지거나 불완전하게 변환되어 AI가 고객의 말을 정확하게 이해하지 못 하게 되었습니다.

예를 들어 "냉장고와 세탁기를 추가하면 얼마일까요?"라는 고객의 말이 "냉장고와 세탁기를 추가" 까지만 변환이 되어 전송 된다면, AI가 완전히 엉뚱한 답변을 생성하게 됩니다.
또, 완전한 문장이 전해져 변환이 되고 AI를 통해 답변이 생성 되어도 연결이 끊김으로 인해 사용자가 답변을 받지 못하게 되는 경우도 있었습니다.
